...
**IMPORTANT** Demos showing any kind of application will require Databricks Apps. Demos that require any low-latency state (e.g. not analytics use cases) will require Lakebase. Review resources/DATABRICKS_APPS.md before continuing if you will need a Databricks App. Review resources/LAKEBASE.md for Lakebase operational patterns including connection pooling, SP permissions, token refresh, and ephemeral workspace recovery. **IMPORTANT**

**IMPORTANT** If the demo needs AI agents that read/write to Lakebase (e.g. via MAS), deploy a Lakebase MCP Server as a Databricks App and connect it to MAS as an External MCP Server agent. See resources/LAKEBASE_MCP_SERVER.md for the full setup, MAS integration walkthrough, and known gotchas. **IMPORTANT**
...
11) When using Lakebase with Databricks Apps, follow the operational checklist in resources/LAKEBASE.md — especially the resource registration, SP permission grants, and ephemeral workspace recovery steps. Skipping these causes silent data failures.
12) If the demo uses a MAS (Multi-Agent Supervisor) that needs to read/write operational data, deploy a Lakebase MCP Server and wire it into MAS as an External MCP Server agent. See resources/LAKEBASE_MCP_SERVER.md.
