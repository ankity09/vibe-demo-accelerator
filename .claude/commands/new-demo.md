...
# Copy DABs bundle definition and deploy scripts
cp "$SCAFFOLD_DIR"/databricks.yml "$PROJECT_DIR"/
mkdir -p "$PROJECT_DIR"/targets "$PROJECT_DIR"/scripts
cp "$SCAFFOLD_DIR"/targets/*.yml "$PROJECT_DIR"/targets/
cp "$SCAFFOLD_DIR"/scripts/*.py "$PROJECT_DIR"/scripts/
cp "$SCAFFOLD_DIR"/scripts/requirements.txt "$PROJECT_DIR"/scripts/
...
**After collecting all answers**, do TWO things:

**1. Append to `PROJECT_DIR/demo-config.yaml`:**
```yaml
# Phase 2: Infrastructure
infrastructure:
...
**2. Generate DABs target config at `PROJECT_DIR/targets/<profile>.yml`:**

Use the Phase 1 demo name (slugified) as the app name, and the CLI profile as the target name. Derive the Lakebase instance name from the app name (hyphens, not underscores).

```yaml
# Target: <profile> — auto-generated by /new-demo Phase 2
targets:
  <profile>:
    workspace:
      host: <workspace_url>
    variables:
      app_name: "<slugified-demo-name>"
      warehouse_id: "<sql_warehouse_id>"
      catalog: "<catalog>"
      schema: "<schema>"
      lakebase_instance: "<slugified-demo-name>"
      lakebase_database: "<schema>_db"
      mas_tile_id: ""
      genie_space_id: ""
      ka_tile_id: ""
      demo_name: "<demo_name from Phase 1>"
      demo_customer: "<customer_name from Phase 1>"
      workspace_email: "<user email from CLI profile>"
```

Tell the user: "I've created a DABs target config at `targets/<profile>.yml`. This will be used during Phase 8 for automated deployment."
...
Show summary and ask: "Phase 3 complete. Does this look right? One more question before we move on..."

### Step 3.5: Real-time streaming data

Tell the user: "Some demos benefit from a 'Start Live Feed' button that simulates real-time data flowing into Delta Lake — IoT telemetry, fleet tracking, sensor readings, or any time-series data. When the audience sees data appear on screen within seconds, it creates a powerful demo moment."

**3.5a Streaming needed?** — Does this demo need real-time data streaming?
- Options: "Yes — show real-time data flowing in", "No — historical data is enough"
- If NO: skip the rest of 3.5, set `streaming.enabled: false`

**If YES, ask these follow-up questions:**

**3.5b Streaming entities** — What kind of streaming data?
- Propose based on Phase 1 research and Phase 3 entities. Examples by industry:
  - Supply chain: fleet GPS, reefer temperatures, vehicle diagnostics
  - Manufacturing: sensor readings, machine telemetry, production line counts
  - Healthcare: patient vitals, bed sensors, equipment status
  - IoT/Energy: meter readings, weather stations, grid telemetry
  - Financial: transaction feeds, market data, fraud signals
- Ask them to pick 1-3 streaming entity types. Each becomes a stream with its own Delta table and cadence.

**3.5c Streaming scenarios** — What "drama" should the live feed create?
- Explain: "During the live feed, we simulate realistic anomalies that trigger alerts — this creates the 'wow moment' when the AI agent detects and responds in real time."
- Propose scenarios based on their streaming entities:
  - **Fault**: sensor spike / threshold excursion (e.g., temperature above limit)
  - **Warning**: approaching regulatory or operational limit
  - **Depletion**: resource running low (e.g., fuel, battery, inventory)
  - **Deviation**: unexpected change from normal pattern
- Ask them to pick 2-3 scenarios to assign to specific entities.

**3.5d Streaming mode** — ZeroBus or simulation-only?
- Options:
  - "ZeroBus (real Databricks streaming)" — recommended. Requires ZeroBus endpoint URL + SP credentials. Data flows through ZeroBus SDK into Delta Lake.
  - "Simulation only (app-side inserts)" — fallback when ZeroBus isn't available. The app's live feed engine INSERTs via Statement Execution API.
- Default to "Simulation" — it works everywhere. ZeroBus is the upgrade path when the workspace has a ZeroBus endpoint.
- If ZeroBus: ask for the ZeroBus URL (format: `https://<id>.zerobus.<region>.cloud.databricks.com`)

**After collecting streaming answers**, append to `PROJECT_DIR/demo-config.yaml`:
```yaml
# Phase 3.5: Streaming
streaming:
  enabled: true|false
  mode: "simulation"|"zerobus"
  zerobus_url: "<url if zerobus mode, empty otherwise>"
  streams:
    - name: "<stream name>"
  ...
  scenarios:
    - type: "fault|warning|depletion|deviation"
      description: "<what happens>"
      entity: "<which entity/stream>"
  entities:
    - id_prefix: "<e.g., VEH-, SENSOR-, PATIENT->"
      count: 5
      routes:
        - origin: [lat, lon]  # or null for non-geo
          destination: [lat, lon]
          scenario: "<scenario type>"
```

Show summary and ask: "Phase 3 (with streaming) complete. Does this look right?"
...
- **If streaming.enabled:** Also offer: "Live telemetry status bar (streaming KPIs + Start/Stop feed toggle)"
...
- **If streaming.enabled:** Suggest a streaming-specific page using domain-appropriate naming:
  - Supply chain: "Fleet Tracker" or "Live Shipments"
  - Manufacturing: "Live Sensors" or "Equipment Monitor"
  - Healthcare: "Patient Monitor" or "Live Vitals"
  - IoT: "Telemetry Dashboard" or "Live Readings"
...
- **Task 2b (if streaming.enabled):** Generate `PROJECT_DIR/notebooks/04_streaming_setup.py` — Fill the template with: CREATE TABLE statements for streaming Delta tables, entity/route definitions, domain-specific generator functions, and 24h historical backfill logic. If streaming.mode="zerobus", include ZeroBus SDK cells; if "simulation", comment out ZeroBus cells.
- **Task 3:** Generate domain API routes in `PROJECT_DIR/app/backend/main.py` — Add endpoints for each entity (list, detail, filters, CRUD for Lakebase entities). **If streaming.enabled:** Import `LiveFeedEngine` from `backend.core.livefeed`, configure streams/entities from demo-config, add domain-specific query endpoints (e.g., latest positions, sensor status, alerts), and mount the streaming router (`create_streaming_router(engine)`). **CRITICAL: Wire workflow approval side-effects** (Gotcha #26). For each `workflow_type` from Phase 3/4, implement `_execute_workflow_actions(wf_row)` that dispatches domain-specific writes on approve (entity updates, notes, agent_actions records) and `_record_dismiss(wf_row)` for the audit trail. Without this, the Agent Workflows "Approve" button only changes status — no domain actions execute and the demo looks broken. Return `actions_taken` list from the PATCH response. Update frontend `approveWorkflow()`/`dismissWorkflow()` to show flash messages from the response.
...
- **If streaming.enabled:**
  - Enable the `<!-- STREAMING -->` sections in the template (un-hide the telemetry bar)
  - Customize KPI tile labels to match the domain (e.g., "Vehicles Reporting" not "Stream 1")
  - Wire `loadTelemetryStats()` to map stream stats to the correct KPI tile IDs
  - Set up auto-refresh interval (15s) for the streaming page
  - Add domain-specific streaming visualizations as needed (maps, sensor gauges, etc.) — these are demo-specific, not scaffold components
...
Walk the user through deployment step by step. **This stays within the /new-demo flow.** The deploy script (`scripts/deploy.py`) handles most infrastructure steps automatically.

Read the DABs target name from `PROJECT_DIR/demo-config.yaml` → `infrastructure.cli_profile`. This is the `--target` value for all deploy commands.
...
**If streaming.enabled:** Run `notebooks/04_streaming_setup.py` (cells 0-4 only: table creation + SP grants + historical backfill). The ZeroBus cells (5-7) are for live demo sessions only, not deployment. Verify streaming tables exist with historical data.

Report: "Phase A complete — N Delta Lake tables created with X rows."

### Step 8B: Lakebase

Tell the user: "Deploying Phase B — setting up Lakebase. The deploy script handles instance creation, database setup, and schema application automatically."

Run:
```bash
python scripts/deploy.py --target <profile> --step lakebase
```

This automatically:
1. Creates the Lakebase instance (if it doesn't exist) with CU_1 capacity
2. Polls until AVAILABLE state (NOT RUNNING — Gotcha #33)
3. Creates the database (if it doesn't exist)
4. Applies `core_schema.sql` and `domain_schema.sql`

Then seed Lakebase data by running `notebooks/03_seed_lakebase.py` via the workspace notebook UI or MCP tools.
...
6. **Update DABs target config** — write the AI layer IDs into `targets/<profile>.yml` so future deploys include them:
   ```yaml
   # Update the variables section in targets/<profile>.yml:
   mas_tile_id: "<actual_mas_tile_id>"
   genie_space_id: "<actual_genie_space_id>"
   ka_tile_id: "<actual_ka_tile_id>"
   ```
   This ensures `databricks bundle deploy` and `scripts/deploy.py` use the correct IDs on subsequent deploys.

Report: "Phase C complete — Genie Space, MAS, and Lakebase MCP server deployed. Agent IDs written to app.yaml, demo-config.yaml, and DABs target config."

### Step 8D: App Deploy

Tell the user: "Deploying Phase D — deploying the app, registering resources, and granting permissions. The deploy script handles this end-to-end."

**First**, copy config files into app/ for deployment (CRITICAL — only app/ is synced to workspace):
```bash
...
**Then**, run the deploy script to handle template generation, app deployment, resource registration, permissions, and verification in one command:

```bash
python scripts/deploy.py --target <profile> --step template --step deploy --step resources --step permissions --step verify
```

This automatically:
1. Generates `app/app.yaml` from the target variables (template step)
2. Deploys via DABs — syncs code and creates/updates the app (deploy step)
3. Registers all resources via API + redeploys for PGHOST injection (resources step)
4. Grants SP permissions — catalog, warehouse, MAS endpoint, Lakebase tables (permissions step)
5. Prints the app URL and health check instructions (verify step)

If DABs is not available, add `--skip-dabs` to fall back to direct CLI deployment.
